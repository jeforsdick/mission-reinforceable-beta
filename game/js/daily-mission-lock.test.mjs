...
test('locked participant return screen replaces replay language with tomorrow messaging', () => {
  assert.match(html, /id="return-tomorrow-bubble"[\s\S]*Come back tomorrow to play another mission!/);
  assert.match(app, /return-tomorrow-bubble'\)\.hidden = !participantLocked/);
  assert.match(app, /participantLocked \? 'Mission Complete'/);
  assert.match(app, /mission-menu \[data-start-mode\][\s\S]*button\.hidden = participantLocked/);
  assert.doesNotMatch(html, /feel free to play again/i);
});
