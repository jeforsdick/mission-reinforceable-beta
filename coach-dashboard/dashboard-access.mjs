...
async function loadCaseDetails(client, caseIds, cases) {
  ...
  const queries = await Promise.all([
    ...
    client.from('game_sessions').select('id, case_id, mission_id, mission_title, started_at, ended_at, status, duration_seconds, active_duration_seconds, plan_aligned_count, refine_count, missed_count, total_hints_opened').in('case_id', caseIds).order('started_at', { ascending: false }),
    client.from('game_responses').select('id, session_id, case_id, fidelity_target_id, fidelity_domain, alignment, hint_opened, hint_open_count, created_at').in('case_id', caseIds)
  ]);
