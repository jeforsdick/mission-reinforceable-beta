import { DOMAIN_LABELS, analyzeCase, coachingCopy, sessionPercent, statusFor } from './dashboard-metrics.mjs';
...
function renderDetail(caseId) {
  ...
  $('#targets-list').innerHTML = targetGroups.length ? targetGroups.map(group => `<section class="target-group"><h3>${DOMAIN_LABELS[group.domain]}</h3>${group.targets.map(target => { const rows = analysis.responses.filter(row => row.fidelity_target_id === target.id); const aligned = rows.filter(row => row.alignment === 'plan_aligned').length; const pct = rows.length ? Math.round(aligned / rows.length * 100) : null; return `<div class="target-item"><span>${escapeHtml(target.description)}</span><strong>${pct === null ? 'No linked opportunities' : `${pct}%`}</strong></div>`; }).join('')}</section>`).join('') : empty('No active fidelity targets are available for this case.');
