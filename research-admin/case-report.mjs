...
export function renderCaseReport(item, prepared, fidelity, e, generated = new Date()) {
  ...
  const observations = (item.observation_data?.observations || []).filter(row => row.summary_revision_id);
  ...
  const observationRows = observations.map(row => [row.observation_date, display(row.phase), row.session_number, row.primary_observer_code, row.secondary_observer_code || '—', row.teacher_fidelity_percent ?? '—', row.student_target_behavior_percent ?? '—', row.ioa?.teacher_fidelity_ioa_percent ?? '—', row.ioa?.student_behavior_ioa_percent ?? '—', row.ioa ? (row.ioa.overall_ioa_attention ? 'Needs review' : 'Complete') : 'Not paired', row.summary_observation_note || row.context_note || '—']);
  const phaseCounts = Object.entries(Object.groupBy ? Object.groupBy(observations, x => x.phase) : observations.reduce((a,x)=>((a[x.phase]??=[]).push(x),a),{})).map(([phase, rows]) => [display(phase), rows.length, rows.filter(x => x.ioa).length, `${rows.length ? Math.round(rows.filter(x => x.ioa).length / rows.length * 1000) / 10 : 0}%`]);
  ...
  ${section('Classroom Observations', table(['Date', 'Phase', 'Session', 'Primary observer', 'IOA observer', 'Teacher fidelity %', 'Student target behavior %', 'Teacher IOA %', 'Student IOA %', 'IOA status', 'Notes'], observationRows, e))}
  ${section('IOA Summary', table(['Metric', 'Value'], [['Completed observations', coverage.completed ?? observations.length], ['Paired IOA observations', coverage.ioa ?? observations.filter(x => x.ioa).length], ['Overall IOA coverage', `${coverage.percent || 0}%`], ['Number needing review', observations.filter(x => x.ioa?.overall_ioa_attention).length]], e) + '<h3>By phase</h3>' + table(['Phase', 'Completed', 'Paired', 'Coverage'], phaseCounts, e))}
