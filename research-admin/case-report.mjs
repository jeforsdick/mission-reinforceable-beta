...
export function renderCaseReport(item, prepared, fidelity, e, generated = new Date()) {
  ...
  const observationRows = observations.map(row => [row.observation_date, display(row.phase), row.session_number, row.primary_observer_code, row.secondary_observer_code || '—', row.teacher_fidelity_percent ?? '—', row.student_target_behavior_percent ?? '—', row.ioa?.teacher_fidelity_ioa_percent ?? '—', row.ioa?.student_behavior_ioa_percent ?? '—', row.ioa ? (row.ioa.overall_ioa_attention ? 'Needs review' : 'Complete') : 'Not paired']);
