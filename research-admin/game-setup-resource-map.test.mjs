...
import { RESOURCE_SECTIONS, captureResourceMap, captureResourceOpenSections, hasUnsupportedResourceBlocks, normalizeResourceMap, renderGameCreation, resetMissionAuthoringState, restoreResourceOpenSections, setupFromWorkspace } from './game-creation-ui.mjs';
const admin = await readFile(new URL('./admin.js', import.meta.url), 'utf8');
const workspace = { case:{id:'a',case_code:'CASE-001',student_alias:'Sky'}, missions:[], setup_draft:{setup:{schemaVersion:1,bipBriefing:'Saved plan'}}, resource_draft:{resources:{schemaVersion:1,sections:{bip:{title:'Old title',blocks:[{type:'paragraph',text:'Plan'}]}}}} };

test('Game Setup loads saved briefing above Mission Bank and saves through the existing RPC',()=>{const html=renderGameCreation(workspace);assert.ok(html.indexOf('class="builder-section game-setup"')<html.indexOf('class="mission-bank"'));assert.match(html,/id="bip-briefing"[\s\S]*Saved plan/);assert.deepEqual(setupFromWorkspace(workspace),{schemaVersion:1,bipBriefing:'Saved plan'});assert.doesNotMatch(html,/Weekly Teacher Report|weeklyTargetBehavior|weeklyReplacementBehavior|weeklyTargetRoutine/);assert.match(admin,/research_admin_save_game_setup_draft/);assert.match(admin,/target_setup: state\.setupDraft/);});
test('legacy weekly report setup text is stripped from active authoring state',()=>{const legacy={setup_draft:{setup:{schemaVersion:1,bipBriefing:'Keep this briefing.',weeklyTeacherReport:{targetBehavior:'Substantive old target',replacementBehavior:'Substantive old replacement',targetRoutine:'Substantive old routine'}}}};const setup=setupFromWorkspace(legacy);assert.deepEqual(setup,{schemaVersion:1,bipBriefing:'Keep this briefing.'});assert.equal(Object.hasOwn(setup,'weeklyTeacherReport'),false);assert.doesNotMatch(renderGameCreation({...workspace,...legacy}),/Substantive old target|Substantive old replacement|Substantive old routine/);});
...
test('all supported blocks and accessible editing controls render while incomplete drafts remain saveable',()=>{const resources=normalizeResourceMap();resources.sections.bip.blocks=[{type:'paragraph',text:''},{type:'heading',text:''},{type:'list',items:['']},{type:'definitionList',items:[{term:'',definition:''}]},{type:'callout',label:'',text:''}];const html=renderGameCreation(workspace,null,null,undefined,'',{},'',undefined,resources);for(const text of ['Paragraph','Heading','SUBSECTION HEADING','Bullet List','Definition List','Callout','Move Up','Move Down','Remove Block','Add item','Add row','Save Resource Map Draft'])assert.match(html,new RegExp(text));assert.match(admin,/research_admin_save_resource_map_draft/);assert.match(admin,/target_resources: state\.resourceDraft/);assert.doesNotMatch(admin,/localStorage|case_game_content/);});

test('Heading is normalized, supported, safely rendered, and captured in canonical shape',()=>{
  const heading={type:'heading',text:'What you may see'};
  const resources=normalizeResourceMap({sections:{bip:{blocks:[heading]}}});
  assert.deepEqual(resources.sections.bip.blocks,[heading]);
  assert.equal(hasUnsupportedResourceBlocks(resources),false);
  const html=renderGameCreation(workspace,null,null,undefined,'',{},'',undefined,resources);
  assert.match(html,/<option value="paragraph">Paragraph<\/option><option value="heading">Heading<\/option><option value="list">Bullet List/);
  assert.match(html,/data-block-type="heading"[\s\S]*SUBSECTION HEADING/);
  assert.match(admin,/select\.value === 'paragraph' \|\| select\.value === 'heading' \? \{ type: select\.value, text: '' \}/);
  const input={value:'What the plan teaches instead'};
  const element={dataset:{blockIndex:'0',blockType:'heading'},querySelector:()=>input};
  const section={dataset:{sectionKey:'bip'},querySelectorAll:()=>[element]};
  captureResourceMap({querySelectorAll:()=>[section]},resources);
  assert.deepEqual(resources.sections.bip.blocks[0],{type:'heading',text:'What the plan teaches instead'});
});
