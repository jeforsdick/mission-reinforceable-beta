...
export function timelineForCase(item){
  ...
  for(const x of item.observation_data?.observations||[]){if(x.primary_record_id)rows.push({date:x.observation_date,category:'Observation',label:`${x.phase} observation #${x.session_number} completed`});if(x.ioa)rows.push({date:x.observation_date,category:'IOA',label:`Teacher ${x.ioa.teacher_fidelity_ioa_percent??'NC'}%, Student ${x.ioa.student_behavior_ioa_percent??'NC'}%`});}
