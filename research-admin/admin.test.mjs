...
assert.match(ui, /Complete all three checks for the current protected-content version/);
assert.match(ui, /resource_behavior_review[\s\S]*resource_privacy_review[\s\S]*resource_qa_preview/);
assert.match(ui, /done\?'Complete ✓':'Needs review'/);
assert.doesNotMatch(js, /Comparability|mission_bank_comparability|comparability_ready/i);
assert.match(ui, /QA only\. This does not turn the game on or count as study data\./);
