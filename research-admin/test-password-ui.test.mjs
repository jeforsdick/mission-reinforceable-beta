...
assert.equal((js.match(/autocomplete="new-password"/g)||[]).length,4);
...
assert.match(js,/DEMO LOGIN TOOLS/); assert.match(js,/legacy-test-password-form/);
assert.match(js,/\^CASE-DEMO-[\s\S]*\^MR-DEMO-/);
assert.match(js,/case_id: form\.dataset\.caseId/);
assert.match(js,/document\.querySelectorAll\('\.legacy-test-password-form'\)/);
assert.match(js,/setLegacyTestPassword[\s\S]*password !== form\.elements\.confirmation\.value[\s\S]*form\.reset\(\)/);
assert.match(js,/result\.warning \|\| 'Test password set\. You can now use the normal Teacher Login\.'/);
