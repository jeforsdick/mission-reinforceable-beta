...
export const FUNCTIONS = ['Escape / Avoidance', 'Attention', 'Tangible / Access', 'Automatic / Sensory', 'Multiple', 'Unclear / Still Being Assessed'];
export const DECISIONS = ['The Setup', 'The Pressure', 'The Pivot', 'The Consequence', 'The Finish'];
const RATINGS = [{ score: 10, label: 'PLAN ALIGNED' }, { score: 5, label: 'WORKABLE / REFINE' }, { score: 0, label: 'PLAN DRIFT' }];
...
const choice = score => ({ text: '', consequence: '', wizard: '', explanation: '', score, bipComponent: '', mechanism: '', errorType: '', function: '', next: score === 10 ? 'd2_supported' : score === 5 ? 'd2_wobbly' : 'd2_escalated' });
const scene = decision => ({ scene: '', hint: '', meta: {}, choices: RATINGS.map(({ score }) => ({ ...choice(score), next: decision === 5 ? null : undefined, ending: decision === 5 ? (score === 10 ? 'STRONG' : score === 5 ? 'MIXED' : 'FRAGILE') : undefined })) });
...
export function blankMission(caseCode, type, slot) {
  ...
  for (let decision = 1; decision <= 4; decision += 1) {
    ...
    for (const id of ids) steps[id].choices.forEach((item, index) => { item.next = nextStepId(decision, TRAJECTORIES[index]); delete item.ending; });
  }
  return { id: defaultMissionId(caseCode, type, slot), title: '', expectedSteps: 5, start: 'd1_start', focus: '', routine: '', centralTension: '', functionPressure: [], activeBipComponents: [], bipTargets: [], endings: Object.fromEntries(ENDINGS.map(key => [key, { text: '', wizard: '' }])), steps };
}
export function normalizeMission(value, caseCode, type, slot) {
  const base = blankMission(caseCode, type, slot);
  if (!value || typeof value !== 'object' || Array.isArray(value)) return base;
  const mission = { ...base, ...structuredClone(value), endings: { ...base.endings, ...(value.endings || {}) }, steps: { ...base.steps } };
  for (const [id, template] of Object.entries(base.steps)) {
    const incoming = value.steps?.[id] || {};
    mission.steps[id] = { ...template, ...incoming, meta: { ...template.meta, ...(incoming.meta || {}) }, choices: RATINGS.map(({ score }, index) => ({ ...template.choices[index], ...(incoming.choices?.[index] || {}), score })) };
  }
...
const isStarted = step => Boolean(step?.scene || step?.hint || step?.meta?.fidelityTargetKey || step?.choices?.some(item => item.text || item.consequence || item.wizard || item.explanation || item.mechanism || item.errorType || item.function || item.bipComponent));
...
const selectOptions = (values, value, empty = 'Select…') => `<option value="">${empty}</option>${values.map(item => `<option value="${esc(item)}"${selected(item, value)}>${esc(item)}</option>`).join('')}`;
function choiceCard(item, index, decision) {
  const rating = RATINGS[index];
  return `<fieldset class="choice-card" data-choice="${index}"><legend><strong>${rating.score}</strong> — ${rating.label}</legend><input type="hidden" name="score" value="${rating.score}">${textField('TEACHER ACTION', 'text', item.text)}${textField('IMMEDIATE MODELED CONSEQUENCE', 'consequence', item.consequence)}${textField('WIZARD', 'wizard', item.wizard)}<label>BEHAVIORAL EXPLANATION<textarea name="explanation">${esc(item.explanation)}</textarea></label><label>BIP COMPONENT<select name="bipComponent">${selectOptions(COMPONENTS, item.bipComponent)}</select></label>${textField('MECHANISM', 'mechanism', item.mechanism)}${textField('ERROR TYPE', 'errorType', item.errorType)}<label>FUNCTION<select name="function">${selectOptions(FUNCTIONS, item.function)}</select></label>${decision < 5 ? `<label>NEXT STATE<select name="trajectory">${TRAJECTORIES.map(branch => `<option value="${branch}"${selected(nextStepId(decision, branch), item.next)}>${branch[0].toUpperCase() + branch.slice(1)}</option>`).join('')}</select></label>` : `<label>ENDING<select name="ending">${ENDINGS.map(key => `<option value="${key}"${selected(key, item.ending)}>${key[0] + key.slice(1).toLowerCase()}</option>`).join('')}</select></label>`}</fieldset>`;
}
...
export function renderGameCreation(workspace, selection, mission, nav, message = '') {
  if (!workspace) return '<section id="game-creation" class="panel"><p class="eyebrow">GAME CREATION</p><h2>Mission authoring workspace unavailable</h2><p class="error-message">The protected authoring workspace could not be loaded. Ask a Research Admin to confirm the authoring RPC is available, then try again. No local-file fallback was used.</p></section>';
  return `<section id="game-creation" class="panel browser-authoring"><div class="game-creation-heading"><div><p class="eyebrow">GAME CREATION</p><h1>Author mission drafts</h1><p>AUTHOR → SAVE DRAFT</p></div><button id="back-to-game-ready" class="quiet" type="button">Back to Game Ready</button></div>${renderMissionBank(workspace, selection)}<section class="resource-map-placeholder"><h2>Resource Map</h2><p>Browser editor coming next.</p>${workspace.resource_draft || workspace.latest_resource_draft ? '<strong>Draft exists</strong>' : ''}</section>${renderMissionBuilder(workspace, selection, mission, nav, message)}<p class="legacy-note">Legacy local build instructions remain available in documentation during the transition.</p></section>`;
}

export function captureMission(root, mission, nav) {
  const one = name => root.querySelector(`[name="${name}"]`);
  for (const name of ['id', 'title', 'routine', 'centralTension', 'focus']) if (one(name)) mission[name] = one(name).value;
  mission.functionPressure = [...root.querySelectorAll('[name="functionPressure"] option:checked')].map(option => option.value);
  mission.activeBipComponents = [...root.querySelectorAll('[name="activeBipComponents"]:checked')].map(input => input.value);
  ...
  if (editor) {
    const step = mission.steps[editor.dataset.stepId]; step.scene = editor.querySelector('[name="scene"]').value; step.hint = editor.querySelector('[name="hint"]').value;
    ...
    editor.querySelectorAll('.choice-card').forEach((card, index) => { const item = step.choices[index]; for (const name of ['text', 'consequence', 'wizard', 'explanation', 'bipComponent', 'mechanism', 'errorType', 'function']) item[name] = card.querySelector(`[name="${name}"]`).value; item.score = RATINGS[index].score; if (nav.decision < 5) { item.next = nextStepId(nav.decision, card.querySelector('[name="trajectory"]').value); delete item.ending; } else { item.next = null; item.ending = card.querySelector('[name="ending"]').value; } });
  }
