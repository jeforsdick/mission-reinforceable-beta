...
export function setupFromWorkspace(workspace) {
  ...
  return { ...structuredClone(setup), schemaVersion: 1, bipBriefing: typeof setup.bipBriefing === 'string' ? setup.bipBriefing : '', weeklyTeacherReport: { ...(setup.weeklyTeacherReport || {}), targetBehavior: typeof setup.weeklyTeacherReport?.targetBehavior === 'string' ? setup.weeklyTeacherReport.targetBehavior : '', replacementBehavior: typeof setup.weeklyTeacherReport?.replacementBehavior === 'string' ? setup.weeklyTeacherReport.replacementBehavior : '', targetRoutine: typeof setup.weeklyTeacherReport?.targetRoutine === 'string' ? setup.weeklyTeacherReport.targetRoutine : '' } };
}
...
export function renderGameSetup(setup, message = '') {
  const weekly = setup?.weeklyTeacherReport || {};
  return `<section class="builder-section game-setup" aria-labelledby="game-setup-title"><p class="eyebrow">GAME SETUP</p><h2 id="game-setup-title">Game Setup</h2><p><strong>BIP Briefing shown before missions</strong></p><p>This is the short case-specific plan summary shown immediately before a teacher begins a mission.</p><p class="privacy-warning">${privacyWarning}</p><label>BIP Briefing<textarea id="bip-briefing" name="bipBriefing" rows="7">${esc(setup?.bipBriefing)}</textarea><small>Write a brief, teacher-friendly reminder of the function and the most important plan actions. Use the approved student alias only.</small></label><section class="weekly-labels"><h3>WEEKLY CHECK-IN LABELS</h3><p>These labels identify the behaviors and routine shown in the teacher's weekly check-in.</p>${textField('TARGET BEHAVIOR', 'weeklyTargetBehavior', weekly.targetBehavior)}${textField('REPLACEMENT OR DESIRED BEHAVIOR', 'weeklyReplacementBehavior', weekly.replacementBehavior)}${textField('TARGET ROUTINE', 'weeklyTargetRoutine', weekly.targetRoutine)}</section><div class="save-bar"><button id="save-game-setup" class="primary" type="button">Save Game Setup</button><p id="setup-save-message" class="message" role="status">${esc(message)}</p></div></section>`;
}
