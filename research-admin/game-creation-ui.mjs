...
export function renderMissionBank(workspace, selection, importer = {}, caseOperations) {
  const eligible = Boolean(workspace?.case?.id && workspace.case.case_code === 'CASE-998' && caseOperations?.study_id === 'MR-998');
  const importPanel = eligible ? `<section class="example-game-import" aria-labelledby="example-import-title"><p class="eyebrow">EXAMPLE GAME IMPORT</p><h3 id="example-import-title">EXAMPLE GAME IMPORT</h3><p>CASE-998 only. Import prepared mission drafts into the Mission Bank.</p><div class="actions"><label class="file-button">Choose JSON File<input id="example-mission-file" type="file" accept=".json,application/json"></label><button id="review-example-import" type="button">Review Import</button><button id="run-example-import" class="primary" type="button"${importer.review?.valid && !importer.importing ? '' : ' disabled'}>Import Mission Drafts</button></div><p id="example-import-message" class="message" role="status">${esc(importer.message || '')}</p>${importer.review ? (importer.review.valid ? `<div class="import-review"><strong>${importer.summary.count} missions ready</strong><p>${importer.summary.ranges.map(esc).join('<br>')}</p><p><strong>Existing slots that will receive a NEW revision:</strong><br>${importer.summary.existing.length ? importer.summary.existing.map(esc).join(', ') : 'None'}</p></div>` : `<div class="import-errors" role="alert"><strong>Import blocked. No missions were saved.</strong><ul>${importer.review.errors.map(error => `<li>${esc(error)}</li>`).join('')}</ul></div>`) : ''}</section>` : '';
  return `<section class="mission-bank" aria-labelledby="mission-bank-title"><h2 id="mission-bank-title">MISSION BANK</h2>${importPanel}${TYPES.map(group => `<section class="mission-bank-group"><h3>${group.label} Missions</h3>${group.type === 'crisis' && !workspace.has_crisis_plan ? '<p class="crisis-label"><strong>Formal crisis plan not present.</strong><br>Do not author crisis procedures that are not in the approved plan. Elevated, safe scenarios may be drafted only within the Mission Authoring Standard.</p>' : ''}<div class="mission-slots">${Array.from({ length: group.count }, (_, index) => { const slot = index + 1, row = latestDraft(workspace, group.type, slot), mission = missionFromDraft(row), active = selection?.mission_type === group.type && selection?.slot_number === slot; return `<button type="button" class="mission-slot${active ? ' selected' : ''}" data-mission-type="${group.type}" data-slot-number="${slot}" aria-pressed="${active}"><strong>${group.label} ${slot}</strong>${mission?.title ? `<span>${esc(mission.title)}</span>` : ''}<small>${row ? `Draft${row.created_at ? ` · saved ${esc(dateLabel(row.created_at))}` : ''}` : 'Not started'}</small></button>`; }).join('')}</div></section>`).join('')}</section>`;
}
