...
const renderedGameCreation=renderGameCreation(readyFixture(),{},x=>String(x));
assert.match(renderedGameCreation,/Build, check, and prepare the individualized game/);
for(const heading of ['1. Create the private case workspace','2. Add the case setup and fidelity targets','3. Build the Resource Map','4. Write the individualized missions','5. Check the game files','6. Create the protected game file','7. Load the protected game into Supabase','8. Preview and review the game','9. Prepare the teacher','10. Start the game when intervention begins']) assert.match(renderedGameCreation,new RegExp(heading.replace(/[.]/g,'\\.')));
for(const label of ['Study ID','Case','Student alias']) assert.match(renderedGameCreation,new RegExp(`<dt>${label}</dt>`));
for(const section of ['BIP at a Glance','Function Forest','Prevention Palace','Replacement Reservoir','Reinforcement Ridge','Error Correction Canyon','BSP Library','Coaching Cottage','Fidelity Fortress']) assert.match(renderedGameCreation,new RegExp(section));
assert.match(renderedGameCreation,/FICTIONAL TRAINING EXAMPLES — NOT PARTICIPANT DATA/);
assert.match(renderedGameCreation,/Before clicking Run,[\s\S]*case code in the SQL matches/);
assert.match(renderedGameCreation,/Run these commands locally/);
assert.match(renderedGameCreation,/Do NOT create the case folder yourself\. The starter command will create it\./);
assert.match(renderedGameCreation,/secure parent folder is[\s\S]*MissionReinforceablePrivate[\s\S]*command will create[\s\S]*CASE-998/);
assert.doesNotMatch(renderedGameCreation,/<li>Create a folder for the current case\.<\/li>/);
assert.match(renderedGameCreation,/Record the intervention phase change using Phase Decision\./);
assert.match(renderedGameCreation,/Game-access, production login, and reminder launch controls will be added here before production study launch\./);
assert.match(renderedGameCreation,/Do not use ad hoc activation steps from this guide\./);
assert.doesNotMatch(renderedGameCreation,/Use the existing, separate phase, account, access, and reminder controls/);
assert.match(renderedGameCreation,/--case-code CASE-TEST/);
assert.match(renderedGameCreation,/Suggested next content version: 1/);
assert.match(renderedGameCreation,/--source-dir "\[PRIVATE CASE FOLDER\]"[\s\S]*--output "\[PRIVATE CASE FOLDER\]\/protected-seed\.sql"/);
assert.match(renderedGameCreation,/Expected first version:<\/strong> 1/);
...
assert.match(renderedGameCreation,/class="quiet inline-action go-teacher-account"[^>]*>Go to Teacher Account<\/button>/);
assert.doesNotMatch(renderedGameCreation,/href="#intake-panel"/,'teacher-account navigation is not a link to the hidden intake panel');
assert.match(js,/\.go-teacher-account[\s\S]*selectCaseTab\('intake'\)[\s\S]*#intake-accounts[\s\S]*scrollIntoView/);
assert.doesNotMatch(renderedGameCreation,/Build it early\. Turn it on only when intervention starts\./);
...
assert.deepEqual([...subnav.matchAll(/<a href="([^"]+)">([^<]+)<\/a>/g)].map(([,href,label])=>[label,href]),[['Overview','#operations-overview'],['Phase Decision','#operations-phase-decision'],['Enrollment','#operations-enrollment'],['Prebaseline','#operations-prebaseline'],['Baseline','#operations-baseline'],['Game Ready','#operations-game-ready'],['Intervention','#operations-intervention'],['End Measures','#operations-end-measures'],['Maintenance','#operations-maintenance'],['Closeout','#operations-closeout'],['Events','#operations-events'],['History','#operations-history']]);
...
const versionedMarkup=renderGameCreation({...launchReady,case_code:'CASE-099'},preparedForLaunch,x=>String(x));
