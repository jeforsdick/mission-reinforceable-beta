...
function mission(id) {
  const steps = {};
  for (let n = 1; n <= 5; n++) steps[`s${n}`] = { text: `Fictional scene ${n}`, hint: 'Review the approved plan.', meta: {}, choices: Object.fromEntries([[10, 'A'], [5, 'B'], [0, 'C']].map(([score, key]) => [key, { text: 'Teacher action', consequence: 'Fictional outcome', wizard: 'Wizard response', feedback: 'Behavioral explanation', score, next: n < 5 ? `s${n + 1}` : null, ...(n === 5 ? { ending: score === 10 ? 'STRONG' : score === 5 ? 'MIXED' : 'FRAGILE' } : {}), meta: { bipComponent: 'Prevent', mechanism: 'Brief prompt', errorType: score === 10 ? 'none' : 'missed_prevention_opportunity', function: 'escape' } }])) };
  return { id, title: 'Fictional mission', routine: 'Class routine', start: 's1', expectedSteps: 5, steps, endings: { STRONG: { text: 'Strong narrative' }, MIXED: { text: 'Mixed narrative' }, FRAGILE: { text: 'Fragile narrative' } } };
}
...
test('setup and specific mission slots are blocking completeness requirements', () => { const value = workspace(); value.setup_draft.setup.bipBriefing = ''; value.missions = value.missions.filter(row => !(row.mission_type === 'daily' && row.slot_number === 4)); const report = validateFullDraft(value); assert.equal(report.ready, false); const messages = Object.values(report.categories).flatMap(category => category.errors.map(item => item.message)); assert(messages.includes('Daily 4 — Not started.')); assert(messages.some(message => /BIP Briefing/.test(message))); assert.doesNotMatch(messages.join(' '),/weekly check-in|Target Routine/i); });
...
test('expectedSteps 5 with five-decision terminal paths passes final mission checks', () => { const report = validateFullDraft(workspace()); assert.equal(report.categories['MISSION STRUCTURE'].errors.length, 0); assert.equal(report.ready, true); });
