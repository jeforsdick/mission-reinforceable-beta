...
function renderStudyOverview() {
  ...
  const visibleCases=visibleDashboardCases(state.operations.cases,state.testCases,{showArchived:state.showArchivedCases,showTest:state.showTestCases});
...
function bindOperations(){const caseId=state.readiness?.case?.id;if(!caseId)return;
 ...
 document.querySelectorAll('#enable-daily-reminders, #disable-daily-reminders, .reminder-setting-action').forEach(button=>button.addEventListener('click',()=>{const enabled=button.dataset.enabled?button.dataset.enabled==='true':button.id==='enable-daily-reminders';const prompt=enabled?'Enable daily Mission: Reinforceable reminders for this teacher?\n\nThis changes reminder eligibility but does not send an email now.':'Disable daily Mission: Reinforceable reminders for this teacher?\n\nThis changes reminder eligibility but does not send an email now.';if(window.confirm(prompt))setTeacherReminders(caseId,enabled);}));
 $('#toggle-test-participant')?.addEventListener('click',async event=>{const currentlyTest=event.currentTarget.dataset.isTest==='true',next=!currentlyTest,label=next?'mark this participant as TEST':'return this participant to REAL study status';if(!window.confirm(`Confirm: ${label}?\n\nTest participants are excluded from production reminders and dissertation outcomes. Real participants follow normal reminder eligibility.`))return;await operationRpc('research_admin_set_test_participant',{target_case_id:caseId,target_is_test:next});});
 $('#simulate-test-reminder')?.addEventListener('click',async()=>{const {data,error}=await state.client.rpc('research_admin_simulate_test_reminder',{target_case_id:caseId});const output=$('#simulate-test-reminder-result');if(output)output.textContent=error?error.message:`Result: ${String(data?.outcome||'unknown').replaceAll('_',' ')}${data?.reason?` — ${data.reason}`:''}`;});
 $('#weekly-qualtrics-form')?.addEventListener('submit',async event=>{event.preventDefault();const output=$('#weekly-qualtrics-result');try{await adminApi('/api/research-admin-communication-readiness',{action:'save_weekly_qualtrics_url',case_id:caseId,weekly_qualtrics_url:new FormData(event.currentTarget).get('weekly_qualtrics_url')});await openDetail(state.selected.request_id,state.selectedTab);}catch(error){output.textContent=error.message;}});
 $('#preview-weekly-email')?.addEventListener('click',async()=>{const output=$('#weekly-email-result');try{const result=await adminApi('/api/research-admin-communication-readiness',{action:'preview_weekly_email',case_id:caseId});const preview=window.open('','_blank');if(!preview)throw new Error('Allow pop-ups to preview the email.');preview.opener=null;preview.document.write(result.html);preview.document.close();}catch(error){output.textContent=error.message;}});
 $('#send-weekly-test')?.addEventListener('click',async event=>{if(!window.confirm('Send this weekly recap only to the configured server-side test recipient?'))return;event.currentTarget.disabled=true;const output=$('#weekly-email-result');try{const result=await adminApi('/api/research-admin-communication-readiness',{action:'send_weekly_test',case_id:caseId});output.textContent=`Test sent to ${result.recipient}.`; }catch(error){output.textContent=error.message;event.currentTarget.disabled=false;}});
