import { accountState, normalizeTargets } from './admin-model.mjs';
...
async function openDetail(id) {
  ...
  try {
    const [teacher, coach] = await Promise.all([exactAccount(row.teacher_email, 'teacher'), exactAccount(row.coach_email, 'coach')]);
    ...
    $('#detail').innerHTML = `<div class="hero"><div><p class="eyebrow">Practitioner-submitted intake/context</p><h1>${escapeHtml(row.teacher_name)} · ${escapeHtml(row.student_initials)}</h1><p>Request ${escapeHtml(row.request_id)} · <span class="pill">${escapeHtml(row.status)}</span></p></div><div class="safeguard"><strong>Review, not approved BIP content</strong><span>The BIP/BSP remains the source of truth for individualized game content and final fidelity targets.</span></div></div>
      ...
      </div><aside><section class="panel"><p class="eyebrow">Account readiness</p><h2>Verified exact-email matches</h2>${accountBox('Teacher Auth account', teacher)}${accountBox('Coach Auth account', coach)}<p>No invitation or password-reset email will be sent.</p></section><section class="panel"><p class="eyebrow">Provisioning</p><h2>Review/readiness only</h2><p class="needs">Needs documented legacy schema</p><p>The repository does not establish the required insert columns for <code>cases</code> and <code>participants</code>. Unsafe writes are disabled.</p><p><strong>Protected game content:</strong> Not loaded</p><p class="off"><strong>Game Access:</strong> OFF — intervention not activated</p><p class="off"><strong>Daily reminders:</strong> OFF</p></section>${reviewActions(row)}</aside></div>`;
