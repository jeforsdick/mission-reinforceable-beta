...
function renderCounts() {
  ...
  const activeCases = partitionStudyCases(state.operations.cases).active;
  const cards = [['New Intakes', count('submitted')], ['Approved / Preparing', count('approved')], ['Prepared Cases', activeCases.length], ['Intervention Active', activeCases.filter(item => item.current_phase === 'intervention').length]];
...
function renderStudyOverview() {
  const allCases=state.operations.cases||[], groups=partitionStudyCases(allCases), cases=visibleStudyCases(allCases,state.showArchivedCases), studyTasks=state.operations.study_wide_tasks||[];
  const all=groups.active.flatMap(item=>attentionForCase(item).map(reason=>({studyId:item.study_id,reason}))).concat(studyWideAttention(studyTasks).map(reason=>({studyId:'Study-wide',reason})));
