...
function bindOperations(){const caseId=state.readiness?.case?.id;if(!caseId)return;
 ...
 $('#generate-study-status-qa')?.addEventListener('click',async()=>{const output=$('#study-status-qa-result');try{const result=await adminApi('/api/research-admin-study-day-status',{action:'generate_qa',case_id:caseId});output.innerHTML=`No email sent. ${Object.entries(result.urls).map(([key,url])=>`<a href="${escapeHtml(url)}" target="_blank" rel="noopener">${escapeHtml(key.replace('_url','').replaceAll('_',' '))}</a>`).join(' · ')}`;}catch(error){output.textContent=error.message;}});
