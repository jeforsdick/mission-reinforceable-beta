...
function bindObservationControls(caseId){
 ...
 const wireRecordForm=form=>{updatePreviews(form);form.querySelectorAll('.interval-cell').forEach(button=>button.addEventListener('click',()=>{cycleInterval(button);updatePreviews(form);}));form.querySelectorAll('.fidelity-entry input').forEach(input=>input.addEventListener('change',()=>updatePreviews(form)));};
 if(unified){wireRecordForm(unified);const ioa=unified.querySelector('.ioa-observer'),secondary=ioa.querySelector('select');unified.querySelectorAll('[name="ioa_collected"]').forEach(input=>input.addEventListener('change',()=>{const selected=unified.querySelector('[name="ioa_collected"]:checked').value==='yes';ioa.hidden=!selected;secondary.required=selected;if(!selected)secondary.value='';}));unified.querySelector('[name="date"]').addEventListener('change',event=>{const date=event.target.value,phase=(state.caseOperations?.phase_history||[]).filter(x=>x.effective_date<=date).sort((a,b)=>b.effective_date.localeCompare(a.effective_date)||b.recorded_at.localeCompare(a.recorded_at))[0]?.phase;unified.querySelector('.phase-helper strong').textContent=phase?phase[0].toUpperCase()+phase.slice(1):'Not determinable';});unified.addEventListener('submit',async event=>{event.preventDefault();const payload=recordPayload(unified),f=new FormData(unified);if(payload.fidelity.some(x=>!x.status)){window.alert('Select one explicit score for every fidelity item.');return;}const button=unified.querySelector('button.primary');button.disabled=true;const {error}=await state.client.rpc('research_admin_record_classroom_observation',{target_case_id:caseId,target_observation_date:f.get('date'),target_primary_observer_id:f.get('primary'),target_secondary_observer_id:f.get('secondary')||null,target_start_time:f.get('start')||null,target_end_time:f.get('end')||null,target_context_note:f.get('note')||null,submitted_fidelity_scores:payload.fidelity,submitted_student_intervals:payload.intervals,target_observer_note:f.get('observer_note')||null});if(error){window.alert(friendlyBaselineError(error.message));button.disabled=false;return;}await loadIntakes();await openDetail(state.selected.request_id,state.selectedTab);if(f.get('secondary')){const waiting=[...document.querySelectorAll('.observation-history li')].find(x=>x.textContent.includes('Waiting for IOA entry'));waiting?.scrollIntoView({behavior:'smooth',block:'center'});window.alert('Observation saved. Enter IOA Data is available below.');}});}
 document.querySelectorAll('.observation-record-form[data-observation]').forEach(form=>{wireRecordForm(form);form.addEventListener('submit',event=>{event.preventDefault();const payload=recordPayload(form),f=new FormData(form);if(payload.fidelity.some(x=>!x.status)){window.alert('Select one explicit score for every fidelity item.');return;}operationRpc('research_admin_submit_classroom_observation_record',{target_observation_id:form.dataset.observation,target_observer_role:form.dataset.role,submitted_fidelity_scores:payload.fidelity,submitted_student_intervals:payload.intervals,target_observer_note:f.get('observer_note')||null,target_correction_reason:f.get('correction_reason')||null});});});
}
...
function updateComparabilityConfirmation() {
  ...
  if (submit) { submit.disabled = !allAnswered || (allPass && (!state.readiness.mission_bank_comparability.complete_bank || !$('#final-confirmation')?.checked)); submit.textContent = allPass && state.readiness.mission_bank_comparability.complete_bank ? 'Approve Mission Bank' : 'Save Review'; }
}
...
function comparabilityPanel(data) {
  ...
  const criteria = COMPARABILITY_CRITERIA.map(([key, title, guidance], index) => `<fieldset class="comparability-criterion"><legend>${index + 1}. ${escapeHtml(title)}</legend><p>${escapeHtml(guidance)}</p><div class="criterion-options"><label><input type="radio" name="criterion-${key}" value="pass" required> Pass</label><label><input type="radio" name="criterion-${key}" value="revise"> Revise</label></div><label>Note — optional<textarea name="note-${key}" maxlength="1000" rows="2"></textarea></label><small>Optional. Keep notes brief and do not copy protected student/BIP content into this field.</small></fieldset>`).join('');
  const history = (review.history || []).map(item => {
    ...
    return `<li><strong>${item.all_pass ? 'All Pass' : 'Needs Revision'}</strong> · ${formatDate(item.reviewed_at)} · ${escapeHtml(item.reviewer)} · version ${escapeHtml(item.protected_content_version)}${revised.length ? ` · Revise: ${revised.join(', ')}` : ''}</li>`;
  }).join('');
  return `<section class="panel comparability no-print"><p class="eyebrow">Mission Review</p><h2>Mission Bank Comparability</h2><p><strong>Check that Daily, Mystery, and Crisis differ by scenario—not by how much support they give.</strong></p><p>Game content version ${escapeHtml(data.protected_content.version)}</p><div class="mission-counts"><span>Daily: <strong>${review.daily_count} / 10</strong></span><span>Mystery: <strong>${review.mystery_count} / 5</strong></span><span>Crisis: <strong>${review.crisis_count} / 5</strong></span></div><p class="${status === 'Ready' ? 'ready' : 'needs'}">${escapeHtml(status)}</p>${review.reviewed ? '<p>This version has a final all-Pass comparability signoff. A later protected content version requires a new review.</p>' : `<form id="comparability-form">${criteria}<label>Overall note — optional<textarea id="comparability-overall-notes" maxlength="2000" rows="3"></textarea></label><small>Keep notes brief and do not copy protected student/BIP content into this field.</small><div id="comparability-confirmation" class="final-confirmation" hidden><label><input id="final-confirmation" type="checkbox"> ${COMPARABILITY_CONFIRMATION}</label></div><button id="submit-comparability" class="primary" disabled>Save Review</button><p id="comparability-message" class="message" aria-live="polite"></p></form>`}<details class="review-history"><summary>Review history (${(review.history || []).length})</summary>${history ? `<ol>${history}</ol>` : '<p>No review attempts recorded.</p>'}</details></section>`;
}
