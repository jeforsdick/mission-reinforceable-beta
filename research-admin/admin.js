...
function bindOperations(){const caseId=state.readiness?.case?.id;if(!caseId)return;
 ...
 $('#go-to-tses')?.addEventListener('click',()=>{const row=document.querySelector('.measure-form[data-key="tses_pre"]');if(!row)return;row.closest('#tses-pre-recording').hidden=false;row.classList.add('measure-focus');row.scrollIntoView({behavior:'smooth',block:'center'});row.querySelector('select[name="status"]')?.focus({preventScroll:true});window.setTimeout(()=>row.classList.remove('measure-focus'),1200);});
