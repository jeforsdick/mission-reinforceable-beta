...
function bindObservationControls(caseId){
 ...
 document.querySelectorAll('.edit-summary-form').forEach(form=>form.addEventListener('submit',event=>{event.preventDefault();const f=new FormData(form),payload=recordPayload(form),rpc=form.dataset.initialLegacy==='true'?'research_admin_create_legacy_observation_summary':'research_admin_revise_classroom_observation_summary';operationRpc(rpc,{target_observation_id:form.dataset.observation,target_observation_note:f.get('observation_note')||null,target_ioa_note:f.get('ioa_note')||null,target_correction_reason:f.get('correction_reason'),...payload});}));
}
