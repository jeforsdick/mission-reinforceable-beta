...
-- This database setting is deliberately fail-closed. Production may set the
-- custom Postgres setting to "true" only when its sender/domain is approved.
create function public.teacher_communication_system_ready()
returns boolean language sql stable security definer set search_path=''
as $$ select coalesce(current_setting('app.settings.teacher_reminder_system_enabled', true), 'false') = 'true' $$;
revoke all on function public.teacher_communication_system_ready() from public, anon;
grant execute on function public.teacher_communication_system_ready() to authenticated;

create function public.research_admin_assert_intervention_launch_ready(target_case_id uuid)
...
revoke all on function public.research_admin_assert_intervention_launch_ready(uuid) from public, anon, authenticated;
...
declare target_participant_id uuid; content_version integer; changed timestamptz:=now(); result jsonb;
begin
  if not public.is_research_admin() then raise exception 'research admin required' using errcode='42501'; end if;
...
create function public.research_admin_set_teacher_reminders(target_case_id uuid, target_enabled boolean)
returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
  ...
  perform pg_advisory_xact_lock(hashtext(target_case_id::text));
  if target_enabled then
    if not public.teacher_communication_system_ready() then raise exception 'Production email delivery has not been enabled.' using errcode='55000'; end if;
    select r.participant_id,r.protected_content_version into target_participant_id,content_version from public.research_admin_assert_intervention_launch_ready(target_case_id) r;
  ...
  insert into public.research_intervention_launch_events(case_id,participant_id,action,actor,recorded_at,protected_content_version)
  values(target_case_id,target_participant_id,case when target_enabled then 'reminders_enabled' else 'reminders_disabled' end,auth.uid(),changed,content_version);
  return jsonb_build_object('enabled',target_enabled,'changed_at',changed);
end $$;

revoke all on function public.research_admin_set_intervention_game_access(uuid,boolean), public.research_admin_set_teacher_reminders(uuid,boolean) from public, anon;
grant execute on function public.research_admin_set_intervention_game_access(uuid,boolean), public.research_admin_set_teacher_reminders(uuid,boolean) to authenticated;
