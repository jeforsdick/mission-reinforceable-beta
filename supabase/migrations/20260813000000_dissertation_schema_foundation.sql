...
participant_id uuid not null references public.participants(id),
case_id uuid not null references public.cases(id),
fidelity_target_id uuid,
fidelity_domain text check (
fidelity_domain in ('proactive', 'teaching', 'reinforcement', 'response', 'crisis')
),
constraint game_responses_target_requires_domain check (
fidelity_target_id is null or fidelity_domain is not null
),
mission_id text not null,
...
-- participants, sessions, fidelity targets, or fidelity domains.
...
add constraint fidelity_targets_id_case_domain_key unique (id, case_id, domain);
...
alter table public.game_responses
add constraint game_responses_fidelity_target_case_domain_fkey
foreign key (fidelity_target_id, case_id, fidelity_domain)
references public.fidelity_targets (id, case_id, domain);
...
create policy "Coaches and research admins can read case intake"
