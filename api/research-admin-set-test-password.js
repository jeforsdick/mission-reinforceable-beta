...
async function handler(request, response) {
  ...
  try {
    ...
    if (cases.length !== 1 || !/^CASE-DEMO-/.test(cases[0].case_code || '')) return conflict(response);
    ...
    if (participants.length !== 1 || !/^MR-DEMO-/.test(participants[0].participant_code || '')) return conflict(response);
