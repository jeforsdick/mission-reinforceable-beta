...
test('communication readiness fails closed and never exposes sender configuration',async()=>{
  const original=global.fetch; delete process.env.TEACHER_REMINDER_SYSTEM_ENABLED;
  ...
  assert.equal(status,200);assert.deepEqual(body,{teacher_reminder_system_enabled:false,game_login_email_enabled:false});assert.doesNotMatch(JSON.stringify(body),/resend|sender|domain|secret/i);
  process.env.TEACHER_REMINDER_SYSTEM_ENABLED='true';await handler({method:'GET',headers:{authorization:'Bearer token'}},response);assert.equal(body.teacher_reminder_system_enabled,true);
  global.fetch=original;delete process.env.SUPABASE_URL;delete process.env.SUPABASE_SERVICE_ROLE_KEY;delete process.env.TEACHER_REMINDER_SYSTEM_ENABLED;
});
