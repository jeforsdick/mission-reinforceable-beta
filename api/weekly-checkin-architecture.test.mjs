...
test('MR-998 Research Admin QA generates a hashed weekly token without email',async()=>{
 ...
 process.env.SUPABASE_URL='https://example.supabase.co';process.env.SUPABASE_SERVICE_ROLE_KEY='service-secret';process.env.WEEKLY_TEACHER_CHECKIN_QUALTRICS_URL='https://survey.example/jfe/form/SV_weekly';
 const calls=[];
 global.fetch=async(url,options={})=>{calls.push({url:String(url),options});if(String(url).endsWith('/auth/v1/user'))return {ok:true,json:async()=>({id:'admin-id'})};if(String(url).includes('/profiles?'))return {ok:true,json:async()=>[{id:'admin-id',role:'research_admin',active:true}]};if(String(url).includes('/participants?'))return {ok:true,json:async()=>[{id:'11111111-1111-4111-8111-111111111111',case_id:'22222222-2222-4222-8222-222222222222',participant_code:'MR-998',cases:{case_code:'CASE-998'}}]};if(String(url).endsWith('/rpc/research_admin_generate_weekly_checkin'))return {ok:true,json:async()=>null};throw new Error(`Unexpected request: ${url}`);};
 try{
  ...
  assert.equal(result.statusCode,200);assert.equal(result.body.email_sent,false);assert.match(result.body.qualtrics_url,/^https:\/\/survey\.example\/jfe\/form\/SV_weekly\?mr_weekly_token=/);assert.match(result.body.completion_test_url,/^https:\/\/qa\.mission\.example\/weekly-checkin-complete\/\?token=/);
