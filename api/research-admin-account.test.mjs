...
Object.assign(process.env, { SUPABASE_URL: 'https://db.example', SUPABASE_SERVICE_ROLE_KEY: 'server-secret', TEACHER_GAME_URL: 'https://mission.example.org/game/' });
...
calls = route({ profiles: [{ id: 'teacher-id', email: 'teacher@example.org', role: 'teacher', active: true }], users: [{ id: 'teacher-id', email: 'teacher@example.org' }] }); res = response(); await qaLink({ method: 'POST', headers: { authorization: 'Bearer token' }, body: { request_id: requestId } }, res); assert.equal(res.statusCode, 200); assert.match(res.body.action_link, /^https:/); const linkCalls = calls.filter(call => call.url.includes('generate_link')); assert.equal(linkCalls.length, 1); const linkRequest = JSON.parse(linkCalls[0].options.body); assert.deepEqual(linkRequest, { type: 'magiclink', email: 'teacher@example.org', redirect_to: process.env.TEACHER_GAME_URL }); assert.doesNotMatch(linkRequest.redirect_to, /localhost:3000/); assert.ok(!calls.some(call => /resend|email\/send/i.test(call.url)));
